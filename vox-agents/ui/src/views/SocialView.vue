...
import { outputSummary, reasoningSummary } from '../utils/social-diagnostics'
...
<template>
  <div class="social-page">
    ...
    <div v-else-if="sessionActive" class="social-shell">
      ...
      <aside class="social-details"><h3>Actors</h3><div v-for="actor in actors" :key="actor.id" class="actor-row"><span class="actor-dot" :class="actor.control" /> <span>{{ actor.displayName }}</span><small v-if="actor.control === 'human'">You</small><Dropdown v-else v-model="actor.modelRef" :options="modelOptions" option-label="label" option-value="value" class="actor-model" @update:model-value="changeActorModel(actor, $event)" /></div><p class="text-muted text-small mt-4">Model changes apply to future replies and do not interrupt the current session.</p><section v-if="inspectionEnabled" class="diagnostics-panel"><h3>Diagnostics</h3><p class="text-muted text-small">Shows sanitized decision metadata only. Private reasoning text is not stored or displayed.</p><div class="diagnostic-summary"><span>Calls {{ diagnosticSummary.calls }}</span><span>Passes {{ diagnosticSummary.passes }}</span><span>Failures {{ diagnosticSummary.failures }}</span></div><div v-if="cascades[0]" class="cascade-summary"><strong>Latest cascade</strong><small>{{ cascades[0].state }} · {{ cascades[0].modelRuns }}/{{ cascades[0].maxModelRuns }} calls · {{ cascades[0].committedModelMessages }}/{{ cascades[0].maxCommittedModelMessages }} messages</small></div><div v-for="diagnostic in diagnostics.slice(0, 8)" :key="diagnostic.id" class="diagnostic-row"><strong>{{ diagnostic.actorDisplayName }} · {{ diagnostic.selectedKind || 'skipped' }}</strong><small>{{ diagnostic.modelRef || 'model unavailable' }}</small><small>{{ diagnostic.providerLatencyMs ?? 'n/a' }} ms · {{ diagnostic.retryCount }} retries</small><small>{{ outputSummary(diagnostic) }}</small><small>{{ reasoningSummary(diagnostic) }}</small><small>{{ diagnostic.applicationOutcome || diagnostic.validationOutcome }}</small></div></section></aside>
    </div>
  </div>
</template>
