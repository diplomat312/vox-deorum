...
import type { CivilizationMindReadModel, CivilizationMindWakeRecord, PoliticalMemorySnapshot } from '@/utils/types';
...
/** Render memory provenance IDs only in the explicit developer section. */
function memoryEvidence(memory?: PoliticalMemorySnapshot): string[] {
  if (!memory) return [];
  const records = [...memory.goals, ...memory.commitments, ...memory.beliefs, ...memory.episodes, ...memory.projects];
  return records.flatMap(record => record.evidence.map(reference => `${reference.kind}:${'id' in reference ? reference.id : reference.traceId}`));
}
...
<template>
  <Dialog v-model:visible="dialogVisible" modal :closable="true" :dismissableMask="true" :style="{ width: '86vw', minWidth: '760px' }" @hide="closeInspector">
    ...
    <div v-else-if="selectedCard" class="mind-inspector">
      ...
      <section v-if="selectedCard.source.memory" class="inspector-section"><h4>Political memory</h4><p class="memory-note">Durable semantic state owned by this civilization, separate from authoritative Civ facts.</p><div v-if="selectedCard.source.memory.goals.length" class="memory-block"><strong>Goals</strong><div v-for="item in selectedCard.source.memory.goals" :key="item.id" class="memory-row"><span>{{ item.title }}</span><Tag :value="item.priority" severity="secondary" /></div></div><div v-if="selectedCard.source.memory.projects.length" class="memory-block"><strong>Projects</strong><div v-for="item in selectedCard.source.memory.projects" :key="item.id" class="memory-row"><span>{{ item.title }}</span><Tag :value="item.status" severity="secondary" /></div></div><div v-if="selectedCard.source.memory.commitments.length" class="memory-block"><strong>Commitments</strong><div v-for="item in selectedCard.source.memory.commitments" :key="item.id" class="memory-row memory-column"><span>{{ item.summary }}</span><small>{{ item.kind }} · {{ item.status }} · players {{ item.parties.join(', ') }}</small></div></div><div v-if="selectedCard.source.memory.relationships.length" class="memory-block"><strong>Subjective relationships</strong><div v-for="item in selectedCard.source.memory.relationships" :key="item.counterpartPlayerId" class="memory-row memory-column"><span>Player {{ item.counterpartPlayerId }}</span><small>Trust {{ item.trust }} · Grievance {{ item.grievance }} · Favor {{ item.favor }} · Threat {{ item.threat }}</small></div></div><div v-if="selectedCard.source.memory.beliefs.length" class="memory-block"><strong>Uncertain beliefs</strong><div v-for="item in selectedCard.source.memory.beliefs" :key="item.id" class="memory-row memory-column"><span>{{ item.claim }}</span><small>{{ item.subject }} · confidence {{ item.confidence }}</small></div></div><div v-if="selectedCard.source.memory.episodes.length" class="memory-block"><strong>Important episodes</strong><div v-for="item in selectedCard.source.memory.episodes" :key="item.id" class="memory-row memory-column"><span>{{ item.summary }}</span><small>Turn {{ item.turn }} · {{ item.importance }}</small></div></div><div v-if="!selectedCard.source.memory.goals.length && !selectedCard.source.memory.projects.length && !selectedCard.source.memory.commitments.length && !selectedCard.source.memory.relationships.length && !selectedCard.source.memory.beliefs.length && !selectedCard.source.memory.episodes.length" class="empty-copy">No durable political memory recorded yet.</div></section>
