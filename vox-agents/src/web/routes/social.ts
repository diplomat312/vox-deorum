...
router.get('/diagnostics', async (req: Request, res: Response) => { if (!socialRuntime.isRunning() || !socialRuntime.inspectionAvailable()) { res.status(404).json({ error: 'Developer diagnostics are unavailable' }); return; } const limit = typeof req.query.limit === 'string' && /^\d+$/.test(req.query.limit) ? Number(req.query.limit) : 100; try { res.json({ diagnostics: await socialRuntime.listDecisionDiagnostics(limit) }); } catch (error) { res.status(500).json({ error: errorMessage(error) }); } });
