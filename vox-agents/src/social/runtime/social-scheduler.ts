...
export class SocialScheduler {
  ...
  private async decide(intention: SocialIntention, scope: SocialExecutionScope, abortSignal: AbortSignal): Promise<void> {
    ...
    if (intention.cascadeId) {
      ...
      if (!reservation.allowed) { await this.store.completeIntention(intention.id, `pass:${reservation.reason ?? 'cascade-budget-exhausted'}`); await this.store.insertDecisionDiagnostic({ intentionId: intention.id, actorId: actor.id, actorDisplayName: actor.displayName, modelRef: actor.modelRef ?? null, executionScope: scope, cascadeId: intention.cascadeId ?? null, selectedKind: null, routingRefsJson: null, validationOutcome: 'skipped-budget', applicationOutcome: 'skipped', error: reservation.reason ?? 'cascade-budget-exhausted', providerLatencyMs: null, queueWaitMs: this.queueWaitMs(intention), durationMs: 0, inputTokens: null, outputTokens: null, totalTokens: null, cachedTokens: null, reasoningTokens: null, cost: null, retryCount: 0 }); return; }
    }
    ...
    context.references = await this.legalReferences(actor, context.references); context.decisionToolDefinitions = definitions; context.decisionTools = createSocialDecisionTools(this.toolScope(scope, intention), context.references, definitions); const startedAt = Date.now(); let diagnosticId: string | undefined;
    try {
      const run = await this.runModel(actor, context, allActors.map((candidate) => candidate.displayName), abortSignal, startedAt);
      const diagnostic = await this.store.insertDecisionDiagnostic({ intentionId: intention.id, actorId: actor.id, actorDisplayName: actor.displayName, modelRef: actor.modelRef ?? null, executionScope: scope, cascadeId: intention.cascadeId ?? null, selectedKind: run.decision.kind, routingRefsJson: JSON.stringify(decisionRoutingSummary(run.decision)), validationOutcome: 'validated', applicationOutcome: null, error: null, providerLatencyMs: run.latencyMs, queueWaitMs: this.queueWaitMs(intention), durationMs: Date.now() - startedAt, inputTokens: run.usage?.inputTokens ?? null, outputTokens: run.usage?.outputTokens ?? null, totalTokens: run.usage?.totalTokens ?? null, cachedTokens: run.usage?.cachedTokens ?? null, reasoningTokens: run.usage?.reasoningTokens ?? null, cost: run.usage?.cost ?? null, retryCount: run.retryCount }); diagnosticId = diagnostic.id;
      ...
      const message = error instanceof Error ? error.message : String(error); if (diagnosticId) await this.store.updateDecisionDiagnostic(diagnosticId, { applicationOutcome: 'error', error: message.slice(0, 500), durationMs: Date.now() - startedAt }); else await this.store.insertDecisionDiagnostic({ intentionId: intention.id, actorId: actor.id, actorDisplayName: actor.displayName, modelRef: actor.modelRef ?? null, executionScope: scope, cascadeId: intention.cascadeId ?? null, selectedKind: null, routingRefsJson: null, validationOutcome: 'failed', applicationOutcome: null, error: message.slice(0, 500), providerLatencyMs: Date.now() - startedAt, queueWaitMs: this.queueWaitMs(intention), durationMs: Date.now() - startedAt, inputTokens: null, outputTokens: null, totalTokens: null, cachedTokens: null, reasoningTokens: null, cost: null, retryCount: 0 }); throw error;
    }
  }
  ...
  private async legalReferences(actor: SocialActor, references: SocialReferenceSet): Promise<SocialReferenceSet> {
    ...
    const eligible = (await Promise.all(dmActors.map(async (candidate) => { const available = await Promise.all(inviteRooms.map((room) => this.store.isActiveOrInvitedMember(room.id, candidate.id))); return available.some((value) => !value) ? candidate : undefined; }))).filter((candidate): candidate is NonNullable<typeof candidate> => candidate !== undefined);
    return { ...references, dmActors, groupParticipants, messageRooms, inviteRooms, inviteParticipants: eligible, leaveRooms };
  }
  /** Measure durable wait before a claimed intention began its provider decision. */
  private queueWaitMs(intention: SocialIntention): number { const claimed = intention.claimedAt ? Date.parse(intention.claimedAt) : Date.now(); const created = Date.parse(intention.createdAt); return Number.isFinite(created) ? Math.max(0, claimed - created) : 0; }
