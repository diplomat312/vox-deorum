...
import { defaultSocialModelExecutor, SocialDecisionExecutionError, type InstrumentedSocialModelExecutor, type SocialDecisionObservability, type SocialModelExecutor } from './social-model-executor.js';
...
function diagnosticObservabilityFields(value?: SocialDecisionObservability): Record<string, unknown> { return { outputTokenLimit: value?.outputTokenLimit ?? null, finishReason: value?.finishReason ?? null, rawFinishReason: value?.rawFinishReason ?? null, incompleteReason: value?.incompleteReason ?? null, responseOutputItemCount: value?.responseOutputItemCount ?? null, responseOutputItemTypesJson: value?.responseOutputItemTypesJson ?? null, responseOutputItemSource: value?.responseOutputItemSource ?? null, sdkToolCallCount: value?.sdkToolCallCount ?? null, sdkTextLength: value?.sdkTextLength ?? null, responseFunctionCallDetected: value?.responseFunctionCallDetected === undefined ? null : value.responseFunctionCallDetected ? 1 : 0, outputLimitReached: value?.outputLimitReached === undefined ? null : value.outputLimitReached ? 1 : 0 }; }
