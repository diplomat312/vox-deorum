...
export class CivEnvironmentAdapter implements EnvironmentAdapter<CivSnapshot> {
  ...
  /** Return actor IDs entitled to a factual event without exposing an unmet participant. */
  public eventRecipientActorIds(event: EnvironmentEvent): string[] { if (!isCivCognitionEventType(event.type)) return []; const bindings = this.listBindings(); if (publicCognitionEventTypes.has(event.type)) return bindings.map((binding) => binding.actorId); const hasSource = this.validPlayerId(event.sourcePlayerId); const hasTarget = this.validPlayerId(event.targetPlayerId); const ids = new Set<string>(); for (const binding of bindings) { if ((hasSource && binding.playerId === event.sourcePlayerId) || (hasTarget && binding.playerId === event.targetPlayerId)) { ids.add(binding.actorId); continue; } if (hasSource && hasTarget) { if (this.knowsPlayer(binding.playerId, event.sourcePlayerId) && this.knowsPlayer(binding.playerId, event.targetPlayerId)) ids.add(binding.actorId); } else { const observedPlayerId = hasSource ? event.sourcePlayerId : hasTarget ? event.targetPlayerId : undefined; if (observedPlayerId !== undefined && this.knowsPlayer(binding.playerId, observedPlayerId)) ids.add(binding.actorId); } } return [...ids]; }
  ...
  /** Recognize a concrete Civ player ID before using it for event visibility. */
  private validPlayerId(playerId: number | undefined): playerId is number { return typeof playerId === 'number' && Number.isSafeInteger(playerId) && playerId >= 0; }
}
