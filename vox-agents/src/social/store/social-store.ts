...
export class SocialStore {
  ...
  public async listActors(sessionId: string): Promise<SocialActor[]> { const rows = await this.opened.db.selectFrom('socialActors').selectAll().where('sessionId', '=', sessionId).orderBy('ordinal').execute(); return rows.map((row) => ({ ...row, control: row.control as SocialActor['control'], status: row.status as SocialActor['status'], modelRef: row.modelRef ?? undefined, profile: row.profile ?? undefined })); }
