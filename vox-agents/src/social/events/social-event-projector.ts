...
export class SocialEventProjector {
  public constructor(private readonly canSeeChannel: (channelId: string) => Promise<boolean>, private readonly inspect = false) {}
  ...
  public async project(event: SocialEvent): Promise<SocialEvent | undefined> {
    ...
    if (!this.inspect && !(await this.canSeeChannel(channelId))) return undefined;
