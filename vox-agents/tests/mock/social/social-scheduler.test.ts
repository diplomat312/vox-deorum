...
describe('SocialScheduler', () => {
  ...
  it('should preserve newer human cascade ownership when an older AI continuation coalesces Bob', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'vox-deorum-social-cascade-owner-')); const store = new SocialStore(join(directory, 'scheduler.sqlite')); cleanups.push(async () => { await store.close(); rmSync(directory, { recursive: true, force: true }); });
    const actors: SocialActorType[] = [{ id: 'human', ordinal: 0, control: 'human', displayName: 'Human', sessionId: 'cascade-owner', createdAt: new Date().toISOString(), status: 'active' }, { id: 'alice', ordinal: 1, control: 'model', displayName: 'Alice', modelRef: 'openrouter/test/alice', sessionId: 'cascade-owner', createdAt: new Date().toISOString(), status: 'active' }, { id: 'bob', ordinal: 2, control: 'model', displayName: 'Bob', modelRef: 'openrouter/test/bob', sessionId: 'cascade-owner', createdAt: new Date().toISOString(), status: 'active' }];
    await store.createSession({ id: 'cascade-owner', humanActorId: 'human' }, actors); const events = new SocialEventHub(); const lanes = new Map(actors.map((actor) => [actor.id, new ActorLane()])); let releaseAlice!: () => void; const aliceStarted = new Promise<void>((resolve) => { releaseAlice = resolve; }); let finishAlice!: () => void; const aliceFinished = new Promise<void>((resolve) => { finishAlice = resolve; }); let firstAlice = true; const contexts = new Map<string, string>(); const executor = { decide: vi.fn(async (actor: SocialActorType, context: SocialContextBundle) => { const text = context.messages.map((message) => typeof message.content === 'string' ? message.content : JSON.stringify(message.content)).join('\n'); contexts.set(actor.id, `${contexts.get(actor.id) ?? ''}\n${text}`); if (actor.id === 'alice' && firstAlice) { firstAlice = false; releaseAlice(); await aliceFinished; return { kind: 'reply' as const, content: 'Alice response from A.' }; } return { kind: 'pass' as const }; }) }; const scheduler = new SocialScheduler(store, async () => actors, lanes, events, executor);
    const first = await store.commitHumanMessage({ sessionId: 'cascade-owner', actorId: 'human', channelId: 'world', content: 'A', budget: { maxModelRuns: 8, maxCommittedModelMessages: 4, maxRepliesPerActor: 2, maxWallClockMs: 60_000 } }); scheduler.kick(); await aliceStarted;
    const second = await store.commitHumanMessage({ sessionId: 'cascade-owner', actorId: 'human', channelId: 'world', content: 'B', budget: { maxModelRuns: 8, maxCommittedModelMessages: 4, maxRepliesPerActor: 2, maxWallClockMs: 60_000 } }); const bobIntentionId = second.createdIntentions.find((intention) => intention.actorId === 'bob')?.id; expect(bobIntentionId).toBe(first.createdIntentions.find((intention) => intention.actorId === 'bob')?.id); expect((await store.getIntention(bobIntentionId!))?.cascadeId).toBe('msg:2');
    finishAlice(); await scheduler.waitForIdle(); scheduler.stop(); const bob = await store.getIntention(bobIntentionId!); expect(bob?.cascadeId).toBe('msg:2'); expect(contexts.get('bob')).toContain('A'); expect(contexts.get('bob')).toContain('B'); expect(contexts.get('bob')).toContain('Alice response from A.'); expect((await store.getCascade('msg:1'))?.state).not.toBe('active'); expect((await store.getCascade('msg:2'))?.state).not.toBe('active');
  });

  it('should freeze before context preparation and retain a pre-provider human follow-up', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'vox-deorum-social-freeze-')); const store = new SocialStore(join(directory, 'scheduler.sqlite')); cleanups.push(async () => { await store.close(); rmSync(directory, { recursive: true, force: true }); });
    const actors: SocialActorType[] = [{ id: 'human', ordinal: 0, control: 'human', displayName: 'Human', sessionId: 'freeze', createdAt: new Date().toISOString(), status: 'active' }, { id: 'alice', ordinal: 1, control: 'model', displayName: 'Alice', modelRef: 'openrouter/test/alice', sessionId: 'freeze', createdAt: new Date().toISOString(), status: 'active' }]; await store.createSession({ id: 'freeze', humanActorId: 'human' }, actors); const events = new SocialEventHub(); const lanes = new Map(actors.map((actor) => [actor.id, new ActorLane()])); let injected = false; let injectionDone: Promise<unknown> | undefined; const builder = new InjectingContextBuilder(() => { if (!injected) { injected = true; injectionDone = store.commitHumanMessage({ sessionId: 'freeze', actorId: 'human', channelId: 'world', content: 'B during context preparation', budget: { maxModelRuns: 5, maxCommittedModelMessages: 3, maxRepliesPerActor: 2, maxWallClockMs: 60_000 } }); } }); let aliceCalls = 0; const contexts: string[] = []; const executor = { decide: vi.fn(async (_actor: SocialActorType, context: SocialContextBundle) => { aliceCalls += 1; contexts.push(context.messages.map((message) => typeof message.content === 'string' ? message.content : JSON.stringify(message.content)).join('\n')); return aliceCalls === 1 ? { kind: 'reply' as const, content: 'A response' } : { kind: 'pass' as const }; }) }; const scheduler = new SocialScheduler(store, async () => actors, lanes, events, executor, builder);
    await store.commitHumanMessage({ sessionId: 'freeze', actorId: 'human', channelId: 'world', content: 'A', budget: { maxModelRuns: 5, maxCommittedModelMessages: 3, maxRepliesPerActor: 2, maxWallClockMs: 60_000 } }); scheduler.kick(); while (!injectionDone) await Promise.resolve(); await injectionDone; await scheduler.waitForIdle(); scheduler.stop(); expect(aliceCalls).toBe(2); expect(contexts[1]).toContain('B during context preparation');
  });
});

/** Trigger a concurrent durable human input during context preparation. */
class InjectingContextBuilder extends SocialContextBuilder {
  public constructor(private readonly inject: () => void) { super(); }
  /** Build channel context while injecting the pending human message. */
  public override build(...args: Parameters<SocialContextBuilder['build']>): ReturnType<SocialContextBuilder['build']> { this.inject(); return super.build(...args); }
}
