...
**Capability rule:** there is no per-flavor capability gating. Every caller gets the full chat and deal surface. Presentation admission is wide: stage 7.04 widens the `VoxDeorumOpenDeal` gate to distinct addressable slots below `MAX_CIV_PLAYERS` (matching VP's native `OnOpenPlayerDealScreen`), so a pure observer can open the native trade screen with its real slot bound as `g_iUs`. Authority stays native: an unsupported participant (an observer slot outside `0 <= playerID < GameDefines.MAX_MAJOR_CIVS`) fails its deal actions at the existing gates — native item legality, `inspect-deal`, the proposal legality guard, enactment — with a clean error and no partial writes. That failure is the intended debug-grade behavior, not a gap to close. Declare War remains a separate seat authority and is never available to a pure observer.
