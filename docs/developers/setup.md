...
2. **Models.** The model registry ships in `vox-agents/src/utils/config/defaults.ts` under `llms`, mapping keys like `openai/gpt-5-mini` to a provider and model, with `default` and `embedder` aliases. `vox-agents/config.json` overrides individual entries, but it is gitignored, so a fresh checkout has none and the shipped defaults apply. The framework is provider-agnostic and resolves each agent's model through its `getModel()` hook. You can assign different models to different agents, or different strategists to different players in the same game. See [vox-agents/overview.md](vox-agents/overview.md).
...
The launcher `scripts/vox-deorum.cmd` brings the services up in dependency order (bridge, then MCP server, then vox-agents) and waits for each to publish its port. On exit it shuts them all down gracefully, falling back to a force-kill, and optionally kills CivilizationV.exe too:
