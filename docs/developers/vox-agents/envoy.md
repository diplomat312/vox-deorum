...
In a diplomacy thread, `send-message` streams the model's raw Message argument as it arrives. Once the call is complete, the tool removes any echoed turn and speaker prefix before appending the durable transcript row. The terminal path then removes transient model and tool traffic, inserts every row that committed during the turn into the in-memory cache, and reports those rows to the client. If a turn neither speaks nor takes a terminal action, it streams and archives the shared retry line. A close requested by the diplomat waits for terminal reconciliation, so the stored order always follows the visible conversation: message, any negotiated outcome, then close.
