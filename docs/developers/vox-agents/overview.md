...
`vox-agents/config.json` holds the runtime configuration, including the `llms` block that maps a key like `openai/gpt-5-mini` to a provider and model name, with a `default` alias and an `embedder` alias for embedding models. That file is gitignored, so a fresh checkout has none. The effective values come from `src/utils/config/defaults.ts` merged with whatever the local file overrides; `src/utils/config.ts` performs the load, and `src/utils/config/diff.ts` does the per-entry merge and the diffing the dashboard's Config view shows. Read the defaults module when you need to know what a setting falls back to.

The full built-in model registry remains in `src/utils/config/defaults.ts`. `src/utils/models/rules.ts` is the single table of recommended settings for known provider and model-name patterns. Live model discovery uses it to annotate the choices shown by the Setup wizard, and runtime resolution uses it to synthesize a compatible entry when it encounters a known provider and model ID that is not already registered. An explicit local configuration or built-in default entry always wins, so synthesis is a compatibility net rather than a replacement for the registry. A name that is neither registered nor recognizable as a provider model falls back to the configured default with a warning, which preserves per-agent names that are not model IDs.

The framework is provider-agnostic. OpenRouter, OpenAI, Anthropic, Google, AWS Bedrock, Codex, and OpenAI-compatible endpoints are supported. API-backed providers read keys from `.env`; Codex uses the player's ChatGPT login, and Claude Code uses the installed and signed-in app. The Setup wizard fetches live, provider-reported model lists for API-backed and local providers, then saves the selected model with any rule-derived recommended settings. Codex and Claude Code use bundled static choices: Codex validates the ChatGPT account through its proxy, while listing Claude Code choices does not verify its installation or sign-in. AWS Bedrock remains a manual Settings-page configuration. Agents resolve their model through `getModel()`, so a config can assign different models to different agents, or different strategists to different players in the same game.
