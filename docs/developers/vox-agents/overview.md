...
The managed Codex proxy is pinned and starts lazily. A listener already occupying its port is never adopted, because `/health` and `/ready` do not expose enough identity to verify the required activity contract; startup instead scans upward from the configured port for a free one and fails only once every candidate is occupied. Requests follow the port the scan settled on, which is resolved when each request is sent rather than when the model is built. `CODEX_PROXY_COMMAND` remains a trusted launch override. See [Updating the Codex proxy](codex.md) for the version upgrade procedure.
