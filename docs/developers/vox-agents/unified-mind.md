...
Unified Civ sessions with at least one unified AI seat also attach the generic SocialRuntime to the live game. Unified AI actors receive bounded message-triggered and environment-event opportunities, and their legal social actions are applied through the existing durable social store and Civ environment gateway. Live Civ uses the existing seat VoxContext and model assignment, not the SocialActor model reference, and keeps one cognition lane per civilization. A live social wake may perform bounded support reads, optionally update Current Outlook, and then choose exactly one outward social/environment action or PASS. Compound outward action plans are deferred to a later phase. Native human diplomacy still uses the existing pairwise diplomacy and deal surface; routing arbitrary human UI messages into the broader SocialRuntime is not yet exposed.
