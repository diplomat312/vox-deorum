...
1. On a fresh install, the dashboard redirects you to the four-step **Setup** wizard. Choose how you want to connect, enter an API key or complete an account sign-in, then choose a model and save. API-backed providers and local servers fetch their current model lists. Codex and Claude Code use bundled choices: Codex validates your ChatGPT account through its proxy, while Claude Code does not verify its installation or sign-in when it lists choices. Codex sign-in is handled in the dashboard, which shows the device link and user code. The one-time user code is never written to the logs. Local setup checks the server address you provide. AWS Bedrock is configured through the advanced Settings page instead.
